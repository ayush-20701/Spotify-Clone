...
async function getSongs(folder, subFolder, play) {
    ...
    let data = await fetch(`http://192.168.137.1:3000/Songs/${folder}/${subFolder}/`)
...
async function main() {
    ...
    let mainData = await fetch('http://192.168.137.1:3000/Songs/')
        ...
        if(heading != 'Default') {
            ...
            Array.from(subFolders).forEach(async (sf, index) => { //for each sub-folder (cards)
                ...
                if(heading == 'Popular Artists') {
                    ...
                    let data = await fetch(cardAsArr[0].href)
                    let response = await data.text()
                    // console.log(response);
